...
public class LibroController {
	...
	@GetMapping(path = "/libros/paginasmayor/{n}")
	public List<Libro> getLibrosPaginasMayor(@PathVariable(name="n") int n){
		List<Libro> result = libroService.getLibrosPaginasMayor(n);
		if(result.size()>0) {
			return result;
		} else {
			throw new NotFoundException();
